...
{
    public class ListArticlesHandler : IRequestHandler<ListArticlesQuery, ListArticleItemsResponse>
    {
        public ListArticlesHandler(IArticleRepository articleRepo, ITagRepository tagRepo, IFavoriteRepository favoriteRepo, IUserRepository userRepo, IFollowingRepository followRepo)
        {
        ...
        public async Task<ListArticleItemsResponse> Handle(ListArticlesQuery request, CancellationToken cancellationToken)
        {
            ...
            foreach (var article in articles)
            {
                ...
                bool isFollowing = request.currentUserId != null
                    ? await _followRepo.IsUserFollowing((Guid)request.currentUserId, author.Id)
                    : false;
