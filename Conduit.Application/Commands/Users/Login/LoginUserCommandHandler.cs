...
namespace Conduit.Application.Commands.Users.Login
{
        ...
        ITokenService jwtTokenService) : IRequestHandler<LoginUserCommand,string>
    {
        ...
        {
            var user = await userRepo.GetAuthUser(request.Username, ct);
            if (user == null) throw new UnauthorizedAccessException("Invalid credentials");

            var hashedPassword = await userRepo.GetPasswordById(user.Id);
            var result = hasher.VerifyHashedPassword(user, hashedPassword, request.Password);
